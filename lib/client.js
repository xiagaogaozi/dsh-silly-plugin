window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function callHost(method, payload = {}) {
			...
			if (!response.ok || parsed === null || parsed.ok !== true || parsed.value === void 0) throw new Error(parsed?.value?.error ?? `HTTP ${response.status}`);
			return parsed.value;
		}
		function TIcon(props) {
			const { d, ds, size = 16, viewBox = "0 0 16 16" } = props;
			const paths = ds ? ds.map((p, i) => (0, react.createElement)("path", {
				key: i,
				d: p,
				fill: "currentColor"
...
`;
		function apply(ctx) {
			...
			function TavernSettings() {
				...
				const [chars, setChars] = (0, react.useState)([]);
				const [deleting, setDeleting] = (0, react.useState)(null);
				const fileInput = (0, react.useRef)(null);
				const refreshChars = () => {
					callHost("listCharacters", {}).then((r) => {
				...
				const openWorkspace = (dirName) => {
					if (!dirName) return;
					callHost("openCharacter", { workspaceName: dirName }).then((r) => {
						if (r && r.ok && r.workspaceId) ctx.get("sessions")?.open(r.workspaceId);
						else setError(r && r.error || "无法打开角色工作区");
					}).catch((e) => setError(e instanceof Error ? e.message : String(e)));
				};
				const deleteCharacter = (dirName) => {
					if (!dirName || deleting) return;
					if (!window.confirm("确定删除角色「" + dirName + "」？\n将删除其工作区目录（card/world/regex/scripts）。")) return;
					setDeleting(dirName);
					callHost("deleteCharacter", { workspaceName: dirName }).then((r) => {
						if (r && r.ok) {
							setError(null);
							refreshChars();
						} else setError(r && r.error || "删除失败");
					}).catch((e) => setError(e instanceof Error ? e.message : String(e))).finally(() => setDeleting(null));
				};
				const handleFile = (file) => {
					...
					file.arrayBuffer().then((buf) => {
						...
						if (r && r.ok) {
							setResult(r);
							refreshChars();
							if (r.workspaceId) ctx.get("sessions")?.open(r.workspaceId);
						} else setError(r && r.error || "导入失败");
					}).catch((e) => {
				...
				const statRow = (icon, label) => (0, react.createElement)("div", { className: "tv-stat-row" }, (0, react.createElement)("span", { className: "tv-stat-ic" }, icon), (0, react.createElement)("span", null, label));
				return (0, react.createElement)("div", { className: "tv-settings" }, (0, react.createElement)("div", { className: "tv-head" }, (0, react.createElement)("span", { className: "tv-head-ic" }, (0, react.createElement)(TIcon, {
					d: ICON_MASKS,
					size: 16,
					viewBox: "0 0 24 24"
				})), (0, react.createElement)("span", { className: "tv-head-label" }, "酒馆模式")), (0, react.createElement)("div", {
					...
					onDrop
				}, (0, react.createElement)("div", { className: "tv-dz-ic" }, (0, react.createElement)(TIcon, {
					d: ICON_FOLDER,
					size: 20
				})), (0, react.createElement)("div", { className: "tv-dz-title" }, busy ? "导入中…" : "导入角色卡"), (0, react.createElement)("div", { className: "tv-dz-hint" }, "点击选择角色卡文件，或直接拖拽到这里（PNG / JSON / Lorebook，任意位置）")), (0, react.createElement)("input", {
					ref: fileInput,
					type: "file",
					accept: ".png,.json,.lorebook",
					style: { display: "none" },
					onChange: onInputChange
				}), result && result.ok && (0, react.createElement)("div", { className: "tv-result" }, (0, react.createElement)("div", { className: "tv-msg" }, "已导入「" + result.cardName + "」"), statRow((0, react.createElement)(TIcon, {
					d: ICON_GLOBE,
					size: 14
				}), "世界书: " + (result.worldMerged ? result.worldMerged + " 个" : "无")), statRow((0, react.createElement)(TIcon, {
					ds: ICON_SEARCH,
					size: 14
				}), "正则: " + (result.regexCount || 0) + " 条"), statRow((0, react.createElement)(TIcon, {
					d: ICON_CODE,
					size: 14
				}), "脚本: " + (result.scriptCount || 0) + " 个"), (0, react.createElement)("div", {
					className: "tv-msg",
					style: { opacity: .65 }
				}, "工作区: " + result.workspacePath), (0, react.createElement)("button", {
					className: "tv-open-btn",
					onClick: () => openWorkspace(result.workspacePath ? result.workspacePath.split("\\").pop() || "" : "")
				}, "打开工作区", (0, react.createElement)(TIcon, {
					d: ICON_CHEVRON_R,
					size: 12
				}))), chars.length > 0 && (0, react.createElement)("div", { className: "tv-list" }, (0, react.createElement)("div", { className: "tv-list-title" }, "已导入角色卡（" + chars.length + "）"), chars.map((c) => (0, react.createElement)("div", {
					className: "tv-char-row",
					key: c.dirName || c.name
				}, (0, react.createElement)("span", { className: "tv-char-ic" }, (0, react.createElement)(TIcon, {
					d: ICON_MASKS,
					size: 16,
					viewBox: "0 0 24 24"
				})), (0, react.createElement)("div", { className: "tv-char-info" }, (0, react.createElement)("div", { className: "tv-char-name" }, c.name), (0, react.createElement)("div", { className: "tv-char-meta" }, (c.worldEntries > 0 ? "世界书 " + c.worldEntries + " 条" : "无世界书") + " · 正则 " + c.regexCount + " · 脚本 " + c.scriptCount + (c.importedAt ? " · " + c.importedAt : ""))), (0, react.createElement)("div", { className: "tv-char-actions" }, (0, react.createElement)("button", {
					className: "tv-char-btn",
					onClick: () => openWorkspace(c.dirName || c.name)
				}, "打开"), (0, react.createElement)("button", {
					className: "tv-char-btn tv-danger",
					disabled: deleting === c.dirName,
					onClick: () => deleteCharacter(c.dirName || c.name)
				}, deleting === c.dirName ? "删除中…" : "删除"))))), error && (0, react.createElement)("div", { className: "tv-error" }, error));
			}
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				order: 1e3,
				label: "酒馆模式"
			}, () => (0, react.createElement)(TavernSettings)));
		}
		//#endregion
		exports.apply = apply;
